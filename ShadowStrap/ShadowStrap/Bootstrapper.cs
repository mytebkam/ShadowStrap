...
namespace ShadowStrap
{
    public class Bootstrapper
    {
        ...
        public string BackgroundUpdaterMutexName => $"ShadowStrap-BackgroundUpdater-{_launchMode}";

        public string MutexNamePrefix { get; set; } = "ShadowStrap-Bootstrapper";
        ...
        private async Task<bool> CheckForUpdates()
        {
            ...
            {
                App.Logger.WriteLine(LOG_IDENT, $"More than one ShadowStrap instance running, aborting update check");
                return false;
            }
            ...
            SetStatus(Strings.Bootstrapper_Status_UpgradingShadowStrap);

            try
            {
#if DEBUG_UPDATER
                string downloadLocation = Path.Combine(Paths.TempUpdates, "ShadowStrap.exe");
